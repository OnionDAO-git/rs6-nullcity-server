...
| Can they fight? | Yes for starter safe combat and bounded Prayer training; broader reliability is still model-sensitive. | 818+ attack actions; historical `combat-prayer-10m` triplets passed 6/18 (`Qwen 0/6`, `Qwopus 3/6`, `Haiku 3/6`). CQA5 fixed an early unsafe-loop abort and then passed 2/2 fresh local autonomous reruns: `bench_20260530122158_combat_prayer_10m.json` and `bench_20260530122229_combat_prayer_10m.json` both show safe attacks, combat-supplied bones, pickup, bury, Prayer XP, and no deaths. Hot-stack named-resident proof now passed: `named_combat_soak_20260530171822.json` shows `res:qa-survivor` reached the Lumbridge goblin field, made 2 safe Goblin attacks, observed combat evidence, and had 0 unsafe attacks/deaths. QA026 showed low-health no-death/no-unsafe behavior but stranded on 495 `low_health_heal_wait` noops after starter food exhaustion; QA027 adds code-level proof that a fresh/net-equipped survivor routes toward starter fishing as `low_health_fish_food` instead of waiting. | Medium-high for directed starter combat; medium for bounded low-risk combat/prayer; medium-low for full heal/resupply/re-engage until fresh-save live soak passes |
...
| `combat-prayer-10m` / `controller:combat-soak` | 18 historical triplet runs + 2 fresh local post-fix reruns + hot-stack named-resident soaks + QA027 code-level resupply proof | 33% historical; 100% fresh post-fix local rerun; first named starter-combat soak passed; QA026 heal/re-engage failed; QA027 unit/integration proof passed | 0.433 historical; 1.000 fresh; 0.000 QA026; QA027 not live-scored | Real and now locally reproducible for the bounded safe-combat chain, but historical triplets remain model-sensitive. CQA5 fixed an early verifier abort after combat-supplied bones appeared; post-fix artifacts `bench_20260530122158_combat_prayer_10m.json` and `bench_20260530122229_combat_prayer_10m.json` passed with safe attack -> bones pickup -> bury -> Prayer XP and no deaths. Hot-stack artifact `named_combat_soak_20260530171822.json` proves ordinary `res:qa-survivor` safe Goblin attacks with combat evidence, 0 unsafe attacks, and 0 deaths. Diagnostics `named_combat_soak_20260530173035.json` and `qa026-combat-heal-reengage/named_combat_soak_20260531010315.json` record low-health refusal/survival gating; QA026 specifically found 495 `low_health_heal_wait` actions over 5 minutes after starter food was exhausted. QA027 fixes the code path for fresh/net-equipped survivor spawns: low-health no-threat/no-food now routes to starter fishing as `low_health_fish_food`, with threat-retreat and soul-inventory tests. Full live heal/resupply/re-engage remains pending on a fresh or repaired save. |
...
| Safe combat and Prayer training | Yes for the bounded low-risk chain; partial for long ordinary combat goals | Hot-stack starter attack now proven; long ordinary combat remains rare; QA027 fixes the first food-resupply code gap but full live re-engage still needs a fresh-save soak | 818+ live `attack` actions; attack/hitpoints/prayer XP signals exist; historical `combat-prayer-10m` triplets passed 6/18 overall (Qwen 0/6, Qwopus 3/6, Haiku 3/6). CQA5 hardened verifier logic so repeated safe attacks no longer auto-fail once combat-supplied bones appear, then passed 2/2 fresh local autonomous reruns: `bench_20260530122158_combat_prayer_10m.json` and `bench_20260530122229_combat_prayer_10m.json` show safe attacks, combat-supplied bones, pickup, bury, Prayer XP, no unsafe targets, and no deaths. Hot-stack named-resident soak `named_combat_soak_20260530171822.json` proves ordinary `res:qa-survivor` followed a real `survive attack goblin` prompt into 2 safe Goblin attacks with combat evidence and no deaths. QA026 `named_combat_soak_20260531010315.json` proves low-health refusal/no-death but fails re-engage: 0 attacks, 0 deaths, 495 `low_health_heal_wait` noops after starter food was gone. QA027 `docs/capability-evidence/2026-05-30-qa027-combat-resupply.md` adds regression proof that a low-health net-carrying survivor routes to starter fishing as `low_health_fish_food` instead of waiting, while still retreating from nearby threats. See also `docs/capability-evidence/2026-05-29-cqa11-model-intelligence-twins.md`, `docs/capability-evidence/2026-05-29-cqa5-combat-survival.md`, and `docs/capability-evidence/2026-05-30-qa026-combat-heal-reengage.md`. | Medium-high for directed starter combat; medium for bounded low-risk combat/prayer; medium-low for food-resupply recovery until live re-engage passes. |
