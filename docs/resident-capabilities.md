...
| AP life-force (low-AP ask + fade/top-up/resume) | Partial | Ask + fade live-proven; top-up/resume live artifact still blocked in this sandbox | S1a replay substrate is landed (`attention` ledger replay helper plus `city_attention_credit` before/after timeline fields). S1b live artifact `bench_20260529175058_ap_decay_ask_5m.json` passed score 1 after a root-cause fix for no-floor residents: the resident said a low-AP appeal at `attentionAfter=10`, then logged out with `cause=attention_exhausted` at `attentionAfter=0` (`lowApAskActions=2`, `attentionExhaustedLogouts=2`, `sawAttentionDropToZero=1`). Follow-up substrate now adds `ap-topup-resume-5m` (verifier requires AP jump plus resumed non-logout action) and benchmark-driver AP top-up injection for autonomous proof, but live run is currently blocked by loopback `EPERM` in this sandbox. See `docs/capability-evidence/2026-05-29-s1b-ap-life-force-benchmark-attempt.md`, issue `QA-20260529-009`. | Medium for ask+fade life-force behavior; low-medium for top-up/resume substrate readiness pending loopback-permitted live artifact capture. |
