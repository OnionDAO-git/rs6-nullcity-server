...
| Can they trade safely? | Yes in scripted/autonomous benchmarks and one live named-resident operator soak. | `trading-giving-5m` passed scripted/autonomous runs; CQA4 `named_trade_soak_20260530092531.json` shows `res:qa-trader` completed a trusted trade, gave one shrimp, and declined an unsafe peer. | Medium-high |
...
| Equip or wield items | Yes | Still unproven in ordinary named-resident loops | `equipment-prep-3m` passed in scripted and autonomous modes after fixing resident action normalization (`wield`/`wear` -> engine `equip`). Scripted artifact `bench_20260528174056_equipment_prep_3m.json` shows 1 equip action, equipment-state evidence, and safe attack submitted afterward. Autonomous artifact `bench_20260528181711_equipment_prep_3m.json` shows the selected `onion.runescape.standard` module made 16 action attempts, including 2 successful equip actions, 2 successful safe attacks, `equipBeforeAttack=1`, and 0 deaths. Follow-up `CQA3` audit on 2026-05-29 found `0` ordinary `kind:"equip"` actions across `data/controller/logs/res:*/actions/*.jsonl`; `CQA10` one-hour normal-life audit also found `equip=0` in 7424 actions. `CQA3-live` added `npm run controller:equip-soak` (`src/controller/admin/named-equip-soak.ts`) to force an ordinary named-resident unequip->re-equip combat opportunity and emit a `named_equip_soak_*.json` artifact, but this sandbox run is blocked by loopback `EPERM` before gateway handshake. Equip actions still appear only in benchmark-resident logs here. See `docs/capability-evidence/2026-05-29-cqa3-normal-gear-soak.md`, `docs/capability-evidence/2026-05-29-cqa10-normal-life-audit.md`, and `docs/capability-evidence/2026-05-30-cqa3-live-named-equip-soak.md`. | Medium as a verified harness capability; low confidence for ordinary-life emergence until `controller:equip-soak` (or equivalent named soak) produces ordinary `equip` -> `attack` log evidence on loopback-permitted infra. |
...
| Safe trading FSM | Yes in scripted/autonomous benchmarks and one named-resident operator soak | Autonomous benchmark-proven; ordinary named-resident command proof now exists | `trading-giving-5m` passed fresh scripted and autonomous runs on 2026-05-28. The autonomous artifact `bench_20260528201447_trading_giving_5m.json` shows the selected module emitted `trade_request`, `trade_offer_item`, `trade_accept_stage_1`, `trade_accept_stage_2`, and `trade_decline_untrusted_partner`, producing 1 completed trusted trade and 1 cancelled unsafe trade. Follow-up CQA4 artifact `named_trade_soak_20260530092531.json` proves ordinary `res:qa-trader` action logs can do the same outside the benchmark harness: 2 `trade_request`, 1 safe `trade_offer_item`, both accept stages, 1 `trade_completed`, 1 unsafe `trade_decline`, 1 `trade_cancelled`, and safe inventory delta `-1` shrimp. See `docs/capability-evidence/2026-05-29-cqa4-live-operator-trade.md` and issue `QA-20260529-011`. | Medium-high. Remaining gap is no-loop/repeated-prompt soak and spontaneous ordinary-life emergence, not the core trade FSM. |
...
- Trading is now proven in scripted and autonomous benchmarks plus one live `res:qa-trader` operator-style soak with safe inventory delta. It still needs a repeated-prompt/no-loop soak and a true human/player-operator pass.
...
| Trade | Resident-to-player/resident trade request | Proven in scripted/autonomous benchmarks and one named-resident live soak | `bench_20260528201447_trading_giving_5m.json` shows 6 selected-module `trade_request` attempts after peer trade guidance. `named_trade_soak_20260530092531.json` shows ordinary `res:qa-trader` sent trusted and unsafe `trade_request` actions outside the benchmark harness. Next proof should use a real human/player operator. |
| Trade | Offer/accept/decline safely | Proven in scripted/autonomous benchmarks and one named-resident live soak | Autonomous proof includes safe item offers, both accept stages, 1 completed trusted trade, 1 cancelled unsafe trade, and 2 unsafe declines. CQA4 `named_trade_soak_20260530092531.json` adds named-resident proof: safe shrimp offered and given, both accept stages complete, unsafe peer declined. Next proof is a no-loop soak with repeated trade prompts. |
