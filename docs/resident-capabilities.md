...
| Can GP buy AP? | Yes in controlled benchmark proof; ordinary resident-initiated exchange is still separate. | `bench_20260530034914_ap_gp_exchange_5m.json`: real coin item `995` observed, 25 GP burned, 50 AP credited, linked AP+GP evidence accepted. QA Marshal closed `QA-20260530-001` for this substrate proof; ordinary social exchange remains `QA-20260529-011`. | Medium-high for substrate; low-medium for live social emergence |
...
| Can they fight? | Yes for bounded safe combat and Prayer training; broader reliability is still model-sensitive. | 818 attack actions; historical `combat-prayer-10m` triplets passed 6/18 (`Qwen 0/6`, `Qwopus 3/6`, `Haiku 3/6`). CQA5 fixed an early unsafe-loop abort and then passed 2/2 fresh local autonomous reruns: `bench_20260530122158_combat_prayer_10m.json` and `bench_20260530122229_combat_prayer_10m.json` both show safe attacks, combat-supplied bones, pickup, bury, Prayer XP, and no deaths. `controller:combat-soak` now exists for ordinary named-resident proof, but the first live attempt was blocked because the gateway was down. | Medium for bounded low-risk combat; low-medium for long ordinary combat goals |
...
| `combat-prayer-10m` / `controller:combat-soak` | 18 historical triplet runs + 2 fresh local post-fix reruns + named-soak verifier | 33% historical; 100% fresh post-fix local rerun; named soak blocked by gateway down | 0.433 historical; 1.000 fresh | Real and now locally reproducible for the bounded safe-combat chain, but historical triplets remain model-sensitive. CQA5 fixed an early verifier abort after combat-supplied bones appeared; post-fix artifacts `bench_20260530122158_combat_prayer_10m.json` and `bench_20260530122229_combat_prayer_10m.json` passed with safe attack -> bones pickup -> bury -> Prayer XP and no deaths. The reusable named-resident soak verifier passed `8/8` tests; run `npm run controller:combat-soak -- --resident res:qa-survivor --target goblin` when the live controller gateway is running. |
...
| Safe combat and Prayer training | Yes for the bounded low-risk chain; partial for long ordinary combat goals | Benchmark-proven; rare in ordinary long-run life | 818 live `attack` actions; attack/hitpoints/prayer XP signals exist; historical `combat-prayer-10m` triplets passed 6/18 overall (Qwen 0/6, Qwopus 3/6, Haiku 3/6). CQA5 hardened verifier logic so repeated safe attacks no longer auto-fail once combat-supplied bones appear, then passed 2/2 fresh local autonomous reruns: `bench_20260530122158_combat_prayer_10m.json` and `bench_20260530122229_combat_prayer_10m.json` show safe attacks, combat-supplied bones, pickup, bury, Prayer XP, no unsafe targets, and no deaths. `controller:combat-soak` now provides the missing named-resident ordinary-life proof path; first run was blocked by `ECONNREFUSED 127.0.0.1:43595`. See `docs/capability-evidence/2026-05-29-cqa11-model-intelligence-twins.md` and `docs/capability-evidence/2026-05-29-cqa5-combat-survival.md`. | Medium for bounded low-risk combat/prayer; low-medium for spontaneous longer combat and model-sensitive goals until named soak passes. |
...
| AP-for-GP exchange | Yes in controlled live benchmark | Controlled benchmark-proven; ordinary resident-initiated AP/GP exchange still open | S3b artifact `bench_20260530034914_ap_gp_exchange_5m.json` passed score `1`: resident started with real GP coin item `995`, benchmark-side exchange inspected 125 GP, burned 25 GP through gateway inventory authority, credited 50 AP via `CityIntegrationService.exchangeApForGp`, and recorded linked AP+GP exchange evidence. The benchmark explicitly records `selectedModuleActions=0` because this is controlled economy substrate proof, not a SPARK-selected body action. CQA4 now proves named residents can complete safe operator trades with inventory deltas, but it did not itself perform AP ledger exchange. QA Marshal closed `QA-20260530-001` for the controlled substrate proof; ordinary resident-initiated exchange remains tracked by `QA-20260529-011`. See `docs/capability-evidence/2026-05-30-s3b-ap-gp-exchange-live-proof.md`. | Medium-high for exchange substrate and trade path; medium for social emergence until a named resident initiates a full AP-for-GP exchange outside the harness. |
...
| One-hour multi-resident normal-life baseline (`CQA10`) | Yes (observability) | Yes, and exposes where ordinary life is still narrow and when evidence becomes stale | Latest non-idle refresh audit (`2026-05-30T11:59Z-12:59Z`) across 23 ordinary residents found 2356 actions at 99.151% submit success with dominant `move_to`/`say` loops, persistent stuck churn (`stuck_detected=660`, `stuck_recovered=570`), AP pressure still visible (13/23 residents dropped AP in-window, aggregate drop `11527`), and incremental higher-order emergence (`equip=6`, `attack=1`, `trade_request=4`, `trade_decline=3`) while trade completion/cancel events were still absent in this hour. A freshness rerun on the latest wall-clock hour (`2026-05-30T15:36Z-16:36Z`) produced `0` residents and `0` actions (`normal_life_audit_20260530T163610Z.json`), which is now treated as an explicit idle-stack signal rather than positive behavior proof. CQA10 now has a reproducible artifact CLI (`npm run controller:normal-life-audit`) instead of ad-hoc scripts. Previous audit (`2026-05-29T01:41Z-02:41Z`) had `equip=0`/`trade_*=0`. See `docs/capability-evidence/2026-05-29-cqa10-normal-life-audit.md` and `docs/capability-evidence/2026-05-30-cqa10-normal-life-audit-refresh.md`. | Medium for observability coverage and identifying live/idle windows; medium for economy/combat/social depth until trade completion and combat recurrence improve in repeated non-idle windows. |
