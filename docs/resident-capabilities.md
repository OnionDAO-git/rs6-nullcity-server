...
| Can they fight? | Combat exists, but reliability is low and model-sensitive. | 818 attack actions; `combat-prayer-10m` passed 6/18 overall in current triplet evidence (`Qwen 0/6`, `Qwopus 3/6`, `Haiku 3/6`); see `docs/capability-evidence/2026-05-29-cqa11-model-intelligence-twins.md`. | Low-medium |
...
| Safe combat and Prayer training | Partial | Rare/partial | 818 live `attack` actions; attack/hitpoints/prayer XP signals exist; `combat-prayer-10m` passed 6/18 overall. Fresh CQA11 model-triplet audit confirms hard-task split remains: Qwen 0/6, Qwopus 3/6, Haiku 3/6, with Qwen failures consistently tripping unsafe-loop guards. See `docs/capability-evidence/2026-05-29-cqa11-model-intelligence-twins.md`. | Low-medium; model choice materially affects hard combat reliability. |
...
| Model intelligence | Local Qwen vs Qwopus/Haiku on hard tasks | Triplet evidence captured; AP/GP strategy live proof still blocked | CQA11 consolidated local+paid model artifacts. Easy loops are saturated (all compared profiles pass) while `combat-prayer-10m` remains model-sensitive (`Qwen 0/6`, `Qwopus 3/6`, `Haiku 3/6`). AP/GP hierarchy benchmark substrate exists but live proof remains blocked by sandbox loopback `EPERM`; see `docs/capability-evidence/2026-05-29-cqa11-model-intelligence-twins.md` and issue `QA-20260529-008`. |
