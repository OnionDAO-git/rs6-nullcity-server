...
| **Inference health check** | `npm run controller:inference-audit` | `headline.usableBrainDecisionRate ≥ 0.10` — low rate means Qwen empty returns dominate; reflex still carries heroes |
...
- **F19c / HD-033 F20a**: heroes' Brain (rich conversation) still limited — Qwen3 thinking-mode returns empty ~87.5% of calls (upstream LLM behavior). **S-INFER-1/2 shipped parse salvage** (thinking_cancelled ≠ empty, balanced-brace JSON recovery, reasoning_content tail support) — the `usableBrainDecisionRate` from `npm run controller:inference-audit` gives the precise post-restart picture. Reflex + identity beacon layer still carries demo experience regardless.
...
| Hero says nothing for >5 minutes | Likely `empty_completion` (HD-033 F20a Qwen3 thinking-mode quirk); 87.5% of Brain calls return empty | Restart controller; reflex layer should still produce fallback says + identity beacons within ~30s. If silence persists, suspect a deeper freeze and run smoke script. S-INFER-1/2 improves parse salvage but does not eliminate the upstream empty rate. |
...
- **Tests**: `npm run fin` now passes **3442/3442** (was 1998) — 1444 new tests across economy, inference, memory, goals, NCRI, Storyteller, stuck-churn, trade, and combat-survival work.
...
- **Inference hardening**: S-INFER-1/2 added parse salvage (thinking_cancelled ≠ empty, reasoning_content tail, balanced-brace JSON recovery, token headroom via `maxTokens`); `npm run controller:inference-audit` gives per-resident usable-brain-decision-rate.
