...
> **🗓 UPDATED 2026-05-31**: This doc was originally written 2026-05-25. Major new capabilities shipped since then: `S-STORY-1/2/3a` (Storyteller overseer + dashboard bridge), `S-NCRI-1/2/3/4` (NCRI marketplace + print queue), `S-MEM-1..4` (resident memory system), `S-GOAL-1..4` (goal-as-orientation + admin CLIs), `S-AP-CYCLE-1/2` (no-floor 3000 AP trigger + hero surplus GP), `S-GP-CALIBRATION-1` (goblin coins 3→20/kill), `S-INFER-1..5` (inference parse salvage, prompt trimmer, loosened limits, fully uninterruptible brain + Qwen3→qwopus switch at 92.7% usable), `S-AUDIT-FIX-1..12` (economy correctness), numerous QA + stuck-churn + trade fixes. Test suite is now **3462** passing (was 1998 on 2026-05-25).
...
| **Inference health check** | `npm run controller:inference-audit` | `headline.usableBrainDecisionRate ≥ 0.80` — qwopus runs at 92.7% post-S-INFER-4; if <0.50 check model/endpoint; if <0.10 restart on correct binary |
...
| **HD-033** | **Substantially mitigated by 2026-05-31 S-INFER-3/4/5 + qwopus switch** | Heroes' Brain returns empty 84-100% under load | **F20c CLOSED (watchdog/LLM-timeout); E53 live-verified.** **F20a (Qwen3 empty-returns) now RESOLVED**: default brain switched Qwen3 → qwopus3.5-27b-v3@q4_k_s at 92.7% usable post-S-INFER-4 (was ~0%). S-INFER-3 prompt trimmer + S-INFER-4/5 inference limits close the structural prompt/abort gaps. F20b (44s endpoint queueing) remains acceptable at 75s timeout. |
...
| Hero says nothing for >5 minutes | qwopus brain runs at 92.7%+ (S-INFER-3/4/5 + c7bf5678). Silence is a freeze or endpoint outage — not the old Qwen3 empty-completion problem. | Restart controller; identity beacons fire within ~30s. Confirm qwopus reachable: `curl http://inf.nullcity.ai:1234/v1/models`. SPARK reflex fallback still produces says even if endpoint is down. |
