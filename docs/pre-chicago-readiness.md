...
| **HTTP port 43596 bound** | `curl -s http://127.0.0.1:43596/v1/inbox?human=health-check` | Returns `{"letters": [...]}` JSON | Controller was started without `--letters-http-port=43596` (HD-026). Restart with the flag. |
...
| **All 7 residents alive** | `cat data/controller/memory/res-*/runtime-state.json \| grep deceased` | All show `null` or `undefined` | `npm run controller:revive -- --resident res:<name>` for each dead one. (Codex's `3f042b38` tooling.) |
