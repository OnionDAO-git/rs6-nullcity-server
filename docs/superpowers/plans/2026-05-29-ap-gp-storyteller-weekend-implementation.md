...
| S5b | Open | - | - | - | - | - | - |
| S6a | Open | - | - | - | - | - | - |
| S6b | In Review | codex | 2026-05-29 | src/controller/city-integration/{service.ts,service.test.ts,http-server.ts,http-server.test.ts}; src/controller/storyteller/{cli.ts,cli.test.ts}; docs/superpowers/plans/2026-05-29-ap-gp-storyteller-weekend-implementation.md; docs/superpowers/plans/2026-05-20-runescape-agent-roadmap.md; docs/agent-status.md | 96b86a06+pending | test:service+http-server+city-integration digest suite; test:storyteller CLI/suite; cmd:storyteller:dry-run -- --memory-root; cmd:check:no-ui; cmd:build; cmd:fin | CityIntegrationService now emits live AP/GP events to EconomyEventLog, city:digest reads service events, GET /api/nullcity/economy/digest exposes the read model, and storyteller:dry-run can write digest/summary artifacts from live EconomyEventLog+GoalContractStore; no UI |
...
| CQA4 | In Review | codex | 2026-05-29 | docs/capability-evidence/2026-05-29-cqa4-live-operator-trade.md; docs/resident-capabilities.md; docs/issue-register.md | pending | doc:docs/capability-evidence/2026-05-29-cqa4-live-operator-trade.md; issue:QA-20260529-011 | benchmark trade is proven, but ordinary controller logs/timelines still show zero trade evidence; needs named-resident operator trade soak with inventory delta |
| CQA5 | Blocked | codex | 2026-05-29 | src/controller/benchmarks/tasks/combat-prayer-10m.ts; src/controller/benchmarks/tasks/combat-prayer-10m.test.ts; docs/capability-evidence/2026-05-29-cqa5-combat-survival.md; docs/resident-capabilities.md; docs/issue-register.md | pending | test:src/controller/benchmarks/tasks/combat-prayer-10m.test.ts; cmd:controller:bench(combat-prayer-10m) | live reruns blocked by sandbox loopback `EPERM` (127.0.0.1:43595); run on loopback-permitted infra |
...
| CQA7 | Blocked | codex | 2026-05-30 | src/controller/benchmarks/tasks/memory-route-recall-5m.ts; src/controller/benchmarks/tasks/memory-route-recall-5m.test.ts; src/controller/benchmarks/cli.ts; src/controller/benchmarks/cli.test.ts; docs/capability-evidence/2026-05-30-cqa7-delayed-memory-route-recall.md; docs/resident-capabilities.md; docs/issue-register.md | pending | test:src/controller/benchmarks/tasks/memory-route-recall-5m.test.ts; test:src/controller/benchmarks/cli.test.ts; cmd:controller:bench --task memory-route-recall-5m --mode autonomous --dry-run; issue:QA-20260529-012 | delayed route-recall benchmark substrate is landed, but live artifact proof is blocked in this sandbox by loopback `EPERM` (`127.0.0.1:43595`) |
| CQA8 | Open | - | - | - | - | - | - |
| CQA9 | Open | - | - | - | - | - | - |
...
| D5 | In Review | codex | 2026-05-29 | `../rs6-nullcity-residents-dashboard` packages/server/src/{storyteller.ts,storyteller.test.ts,index.ts,config.ts,config.test.ts}; packages/web/src/{App.svelte,lib/api.ts,app.css} | dashboard:7b1db56+401d657+be2734d | test:bun test 12/12 focused; cmd:bun run typecheck; cmd:bun run check; cmd:bun run build; browser:http://127.0.0.1:5174/story shows grounded event cards with coin-995/NCRI labels and no raw human ids | next: wire live economy digest/read-model into Storyteller run source, then model-backed dispatch artifacts from S7 |
