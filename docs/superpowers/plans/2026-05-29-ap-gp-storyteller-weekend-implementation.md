...
| S7b | In Review | codex | 2026-05-30 | src/controller/storyteller/{model-client.ts,model-client.test.ts,run-cli.ts,run-cli.test.ts}; data/controller/storyteller/s5b-ncri-proof-20260530T0920/dispatch.json; docs/superpowers/plans/2026-05-29-ap-gp-storyteller-weekend-implementation.md; docs/superpowers/plans/2026-05-20-runescape-agent-roadmap.md; docs/agent-status.md | f9bdffd5+pending | test:storyteller run/model client 17/17; test:storyteller suite 164/164; cmd:storyteller:run -- --latest nooped; cmd:live 1h digest fallback | storyteller:run can now narrate fixture, latest persisted digest, or named digest-id artifacts; no endpoint configured writes a reviewable grounded fallback dispatch instead of placeholder title/body copy, while configured profiles record model/profile/cost metadata |
| S8a | Open | - | - | - | - | - | - |
