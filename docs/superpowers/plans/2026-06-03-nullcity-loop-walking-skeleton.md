...
| iter 2 | **T0.0a (Task 2)** | ✅ DONE + pushed | 78/78 city, typecheck clean | `attention_grant_intents` saga (`runAttentionGrant`): intent → labelled non-prod stand-in debit → real `creditAttention` → settled; idempotent; response no longer `mocked`. 003 migration + `creditAttention` client + intent persistence (3 stores). (`dff2ab9`→origin/main). |
| iter 3 | **T0.0b (Task 3)** | ✅ DONE + pushed | 572/572 patron+city, tsc clean | **Seam already existed inline** (`onPatronSupport`, QA-20260601-065) but mapped onions→standing **1:1** and keyed on raw `cityUserId`. Extracted to tested `recordSettledSupport` (Shards-free), added `onionsPerStandingPoint` scale (default 1, **value is a James decision**), keyed on `patronHandle ?? cityUserId`, threaded `patronHandle` through `PatronSupportEvent` + schema. Behavior-preserving. (`347dcdbf`→origin/agents/wip). |
| iter 4 | **T0.4 (Task 4)** | ✅ DONE + pushed | 160/160 dashboard server, 572/572 server, tsc clean | Capstone: `upsertUserFromLanding` auto-populates the `personId→patronHandle` alias (3 stores); `runAttentionGrant` resolves + forwards `personId`/`patronHandle` in the City body. Full chain unit-proven both sides. (`bba0b5a`→origin/main). |
| iter 4 | **ALL IN-OUR-CONTROL SLICES** | ✅ **COMPLETE** | dashboard 160/160 · server 572/572 patron+city · all typecheck clean | The walking skeleton is built end-to-end on our side: Support → real intent saga → labelled stand-in debit → City `creditAttention` → `onPatronSupport` (with `patronHandle`) → Shards-free `recordSettledSupport` → standing + thank-you letter, keyed on the canonical identity. **Remaining = Dev-gated (real spend API) + restart-gated (live letter), both queued below.** |
...
**Queued for Dev (the only thing blocking the *real* loop, not the skeleton):**
