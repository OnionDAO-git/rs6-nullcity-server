...
| iter 4 | T0.4 (Task 4) | next | — | Integration capstone: populate alias on upsert (dashboard, 3 stores) + resolve & forward `personId`/`patronHandle` in `runAttentionGrant` → BFF integration test proving the end-to-end identity flow. Live letter verification = runtime-steward (restart-gated). |
