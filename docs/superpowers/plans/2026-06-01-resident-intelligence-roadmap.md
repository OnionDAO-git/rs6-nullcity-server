...
| 5 — Polish / scale | **In Progress** — S-PLAN-BUDGET-1 (daily per-resident planner call cap MAX=10, +5 tests, sha=834e0f80); RIQ-5-1 (GET /residents/:id/plan observability endpoint, +2 tests, sha=e3fa945f); RIQ-5-2 (planner failure backoff 200-tick cooldown after fail/throw, +5 tests, sha=6995580b); RIQ-5-3 (global concurrent PlannerPass cap MAX=3, prevents burst-N paid calls on restart, +6 tests, sha=TBD). All substrate-only; live-verify PENDING hot stack. Remaining: plan dashboard (dashboard repo), N≥10 soak benchmark. | N≥10 residents, bounded spend, plan dashboard | 2026-06-04 |
