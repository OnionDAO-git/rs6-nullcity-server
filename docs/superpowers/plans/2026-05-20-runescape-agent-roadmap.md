...
- `[>]` **N4: IRL graveyard wall at the embassy.** Printed epitaphs at the physical embassy; refresh cadence + printing pipeline.
  - Started 2026-05-26 on `agents/wip`: add `GET /v1/graveyard` JSON endpoint + printable `/graveyard/` HTML page so event staff can render tombstone cards for the physical embassy wall.
