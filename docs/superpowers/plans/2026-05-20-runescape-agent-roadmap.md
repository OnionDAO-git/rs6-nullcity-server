...
- `[~]` **G5: Follow and command loop.**
  ...
  - Follow-up: add an SDK-over-HTTP MCP integration test for `patron_ask`, matching the live proof path instead of only exercising the registered tool handler directly.
