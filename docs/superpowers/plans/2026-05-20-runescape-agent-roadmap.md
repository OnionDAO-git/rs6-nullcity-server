...
- `[>]` **N5: Mortician's Ribbon civic achievement.** Bestowed for humans witnessing N resident deaths (N TBD). In-game cape/title + lanyard variant.
  - Started 2026-05-25 on `agents/wip`: bundling ribbon dispatch into `dispatchEpitaphs` — each patron who receives an epitaph also gets a Mortician's Ribbon civic milestone letter (threshold N=1 per vision doc).
