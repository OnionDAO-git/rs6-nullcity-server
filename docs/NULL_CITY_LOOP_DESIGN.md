...
> **Status:** DRAFT v0.3.3 — generic `personId` identity decided (D-IDENTITY); trust mechanism firmed; Dev hand-off list in §10
...
**Leg A — Human → Resident (un-mock) — a cross-process saga, settled against the canonical wallet.**
1. Settlement layer (BFF) writes an `attention_grant_intents` row keyed by `idempotencyKey`; **debit onions via the new authenticated landing spend API** (or, interim, the BFF projection mirrored to landing) in one transaction. States: `created → debited → sent_to_city → settled | refunded | clawed_back | failed`.
2. Call City `creditAttention` with the **same** `idempotencyKey` + a **byte-stable** payload to avoid the `409 idempotency_payload_mismatch` trap (`service.ts:1463`).
3. On 504/timeout, treat the credit as **unknown, not failed** (`nullcity-control.ts:245,262`) — only the reconciler resolves it. **Never tell the user "grant sent" before settlement** (fixes `App.svelte:1949`).
4. On settle → trigger Leg D.
5. **Reconciler** (Q12): refund `debited`-but-unsettled past timeout; reconcile City-accepted-but-BFF-unsettled by event id (don't double-grant); operator repair path before public launch.

**D-TRUST-SPLIT — two boundaries, two mechanisms (firmed by security review; the asymmetry follows from *who owns the authoritative check*):**
- **BFF → landing (the scarce debit): scoped, rotatable API-key over TLS — NOT HMAC.** Landing owns the authoritative balance check (locks the row, re-derives `balance>=amount`), so the API-key only needs to answer "are you the BFF?" Reuse landing's existing timing-safe `X-API-KEY` primitive (`api/webhook/notion/+server.ts:10-16`), distinct secret, scoped to onion spend/grant/refund/read only. HMAC here is **overkill** — TLS prevents in-flight tampering, and even a tampered amount is bounded by the real balance and produces a reversible, attributable ledger row. **NON-NEGOTIABLE RIDER:** the API-key only suffices once landing's idempotency stops being `ON CONFLICT DO NOTHING`/returns-null (`points.ts:61,65`) and becomes **payload-mismatch → 409** (mirror City's guard) with a **required UUID `idempotencyKey`**. Without that, there's a real replay gap — and the fix is the ledger guard, not HMAC.
