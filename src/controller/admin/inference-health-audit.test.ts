...
describe('inference health audit', () => {
    ...
    describe('classifyDecisionCause', () => {
        it('maps every salvage class onto a health class (SPARK + brain spellings)', () => {
            ...
            expect(classifyDecisionCause('thinking_cancelled:nervous:witness-requirement-rule')).toBe('thinking_cancelled');
            expect(classifyDecisionCause('thinking_cancelled:interrupted_by:trade_request')).toBe('thinking_cancelled');
    ...
    it('rolls up a mixed-cause fixture per resident and in aggregate', () => {
        ...
        // res:agent — 10 brain-eligible decisions covering every class:
        //   clean x2, recovered x2, think_only x1, cancelled x3, schema x1, empty x1
        // usable = (2 + 2) / 10 = 0.4
        writeTrajectory(trajectoryRoot, 'res:agent', [
            ...
            decision('2026-05-31T17:36:45.000Z', 'thinking_cancelled:nervous:witness-requirement-rule'),
        ...
        expect(report.brainEligibleDecisions).toBe(14);

        // aggregate breakdown: agent(2,2,1,3,1,1) + hero(3,0,0,0,0,1)
        expect(report.breakdown).toEqual({
            clean: 5,
            recovered: 2,
            think_only_no_answer: 1,
            thinking_cancelled: 3,
            schema_mismatch: 1,
            truly_empty: 2,
        });
        // headline: (5 + 2) / 14 = 0.5
        expect(report.usableBrainDecisionRate).toBe(0.5);
        ...
        expect(agent!.brainEligibleDecisions).toBe(10);
        expect(agent!.usableBrainDecisionRate).toBe(0.4);
