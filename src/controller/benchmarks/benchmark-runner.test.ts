...
describe('BenchmarkRunner', () => {
    it('creates a disposable resident, runs a bounded task, records evidence, and cleans up', async () => {
        ...
        expect(gateway.submitActionWithRequestId).toHaveBeenCalledWith('bench:make-fire-5m:run-1', {
            kind: 'say',
            text: 'benchmark hello',
        });
        expect(gateway.disconnectResident).toHaveBeenCalledWith('bench:make-fire-5m:run-1');
        expect(gateway.deleteResident).toHaveBeenCalledWith('bench:make-fire-5m:run-1');
...
function runner(gateway: BenchmarkGateway, task: BenchmarkTask): BenchmarkRunner {
    return new BenchmarkRunner({
        ...
        task,
        runId: 'run-1',
        residentName: 'bench:make-fire-5m:run-1',
